  ...
  ai_msgs__msg__PerceptionTargets__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    ai_msgs__msg__PerceptionTargets * data =
      (ai_msgs__msg__PerceptionTargets *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ai_msgs__msg__PerceptionTargets__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ai_msgs__msg__PerceptionTargets__fini(&output->data[i]);
        }
        return false;
      }
    }
