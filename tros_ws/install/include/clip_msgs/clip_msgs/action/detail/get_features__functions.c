// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from clip_msgs:action/GetFeatures.idl
// generated code does not contain a copyright notice
#include "clip_msgs/action/detail/get_features__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `urls`
// Member `texts`
#include "rosidl_runtime_c/string_functions.h"

bool
clip_msgs__action__GetFeatures_Goal__init(clip_msgs__action__GetFeatures_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // type
  // urls
  if (!rosidl_runtime_c__String__Sequence__init(&msg->urls, 0)) {
    clip_msgs__action__GetFeatures_Goal__fini(msg);
    return false;
  }
  // texts
  if (!rosidl_runtime_c__String__Sequence__init(&msg->texts, 0)) {
    clip_msgs__action__GetFeatures_Goal__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_Goal__fini(clip_msgs__action__GetFeatures_Goal * msg)
{
  if (!msg) {
    return;
  }
  // type
  // urls
  rosidl_runtime_c__String__Sequence__fini(&msg->urls);
  // texts
  rosidl_runtime_c__String__Sequence__fini(&msg->texts);
}

bool
clip_msgs__action__GetFeatures_Goal__are_equal(const clip_msgs__action__GetFeatures_Goal * lhs, const clip_msgs__action__GetFeatures_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // type
  if (lhs->type != rhs->type) {
    return false;
  }
  // urls
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->urls), &(rhs->urls)))
  {
    return false;
  }
  // texts
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->texts), &(rhs->texts)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_Goal__copy(
  const clip_msgs__action__GetFeatures_Goal * input,
  clip_msgs__action__GetFeatures_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // type
  output->type = input->type;
  // urls
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->urls), &(output->urls)))
  {
    return false;
  }
  // texts
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->texts), &(output->texts)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_Goal *
clip_msgs__action__GetFeatures_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Goal * msg = (clip_msgs__action__GetFeatures_Goal *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_Goal));
  bool success = clip_msgs__action__GetFeatures_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_Goal__destroy(clip_msgs__action__GetFeatures_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_Goal__Sequence__init(clip_msgs__action__GetFeatures_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Goal * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_Goal *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_Goal__Sequence__fini(clip_msgs__action__GetFeatures_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_Goal__Sequence *
clip_msgs__action__GetFeatures_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Goal__Sequence * array = (clip_msgs__action__GetFeatures_Goal__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_Goal__Sequence__destroy(clip_msgs__action__GetFeatures_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_Goal__Sequence__are_equal(const clip_msgs__action__GetFeatures_Goal__Sequence * lhs, const clip_msgs__action__GetFeatures_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_Goal__Sequence__copy(
  const clip_msgs__action__GetFeatures_Goal__Sequence * input,
  clip_msgs__action__GetFeatures_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_Goal * data =
      (clip_msgs__action__GetFeatures_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
clip_msgs__action__GetFeatures_Result__init(clip_msgs__action__GetFeatures_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // error_code
  return true;
}

void
clip_msgs__action__GetFeatures_Result__fini(clip_msgs__action__GetFeatures_Result * msg)
{
  if (!msg) {
    return;
  }
  // success
  // error_code
}

bool
clip_msgs__action__GetFeatures_Result__are_equal(const clip_msgs__action__GetFeatures_Result * lhs, const clip_msgs__action__GetFeatures_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // error_code
  if (lhs->error_code != rhs->error_code) {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_Result__copy(
  const clip_msgs__action__GetFeatures_Result * input,
  clip_msgs__action__GetFeatures_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // error_code
  output->error_code = input->error_code;
  return true;
}

clip_msgs__action__GetFeatures_Result *
clip_msgs__action__GetFeatures_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Result * msg = (clip_msgs__action__GetFeatures_Result *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_Result));
  bool success = clip_msgs__action__GetFeatures_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_Result__destroy(clip_msgs__action__GetFeatures_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_Result__Sequence__init(clip_msgs__action__GetFeatures_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Result * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_Result *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_Result__Sequence__fini(clip_msgs__action__GetFeatures_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_Result__Sequence *
clip_msgs__action__GetFeatures_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Result__Sequence * array = (clip_msgs__action__GetFeatures_Result__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_Result__Sequence__destroy(clip_msgs__action__GetFeatures_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_Result__Sequence__are_equal(const clip_msgs__action__GetFeatures_Result__Sequence * lhs, const clip_msgs__action__GetFeatures_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_Result__Sequence__copy(
  const clip_msgs__action__GetFeatures_Result__Sequence * input,
  clip_msgs__action__GetFeatures_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_Result * data =
      (clip_msgs__action__GetFeatures_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `item`
#include "clip_msgs/msg/detail/clip_item__functions.h"

bool
clip_msgs__action__GetFeatures_Feedback__init(clip_msgs__action__GetFeatures_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_progress
  // item
  if (!clip_msgs__msg__ClipItem__init(&msg->item)) {
    clip_msgs__action__GetFeatures_Feedback__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_Feedback__fini(clip_msgs__action__GetFeatures_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_progress
  // item
  clip_msgs__msg__ClipItem__fini(&msg->item);
}

bool
clip_msgs__action__GetFeatures_Feedback__are_equal(const clip_msgs__action__GetFeatures_Feedback * lhs, const clip_msgs__action__GetFeatures_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_progress
  if (lhs->current_progress != rhs->current_progress) {
    return false;
  }
  // item
  if (!clip_msgs__msg__ClipItem__are_equal(
      &(lhs->item), &(rhs->item)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_Feedback__copy(
  const clip_msgs__action__GetFeatures_Feedback * input,
  clip_msgs__action__GetFeatures_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_progress
  output->current_progress = input->current_progress;
  // item
  if (!clip_msgs__msg__ClipItem__copy(
      &(input->item), &(output->item)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_Feedback *
clip_msgs__action__GetFeatures_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Feedback * msg = (clip_msgs__action__GetFeatures_Feedback *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_Feedback));
  bool success = clip_msgs__action__GetFeatures_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_Feedback__destroy(clip_msgs__action__GetFeatures_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_Feedback__Sequence__init(clip_msgs__action__GetFeatures_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Feedback * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_Feedback *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_Feedback__Sequence__fini(clip_msgs__action__GetFeatures_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_Feedback__Sequence *
clip_msgs__action__GetFeatures_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_Feedback__Sequence * array = (clip_msgs__action__GetFeatures_Feedback__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_Feedback__Sequence__destroy(clip_msgs__action__GetFeatures_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_Feedback__Sequence__are_equal(const clip_msgs__action__GetFeatures_Feedback__Sequence * lhs, const clip_msgs__action__GetFeatures_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_Feedback__Sequence__copy(
  const clip_msgs__action__GetFeatures_Feedback__Sequence * input,
  clip_msgs__action__GetFeatures_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_Feedback * data =
      (clip_msgs__action__GetFeatures_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "clip_msgs/action/detail/get_features__functions.h"

bool
clip_msgs__action__GetFeatures_SendGoal_Request__init(clip_msgs__action__GetFeatures_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    clip_msgs__action__GetFeatures_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!clip_msgs__action__GetFeatures_Goal__init(&msg->goal)) {
    clip_msgs__action__GetFeatures_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_SendGoal_Request__fini(clip_msgs__action__GetFeatures_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  clip_msgs__action__GetFeatures_Goal__fini(&msg->goal);
}

bool
clip_msgs__action__GetFeatures_SendGoal_Request__are_equal(const clip_msgs__action__GetFeatures_SendGoal_Request * lhs, const clip_msgs__action__GetFeatures_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!clip_msgs__action__GetFeatures_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_SendGoal_Request__copy(
  const clip_msgs__action__GetFeatures_SendGoal_Request * input,
  clip_msgs__action__GetFeatures_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!clip_msgs__action__GetFeatures_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_SendGoal_Request *
clip_msgs__action__GetFeatures_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_SendGoal_Request * msg = (clip_msgs__action__GetFeatures_SendGoal_Request *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_SendGoal_Request));
  bool success = clip_msgs__action__GetFeatures_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_SendGoal_Request__destroy(clip_msgs__action__GetFeatures_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__init(clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_SendGoal_Request * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_SendGoal_Request *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__fini(clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_SendGoal_Request__Sequence *
clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * array = (clip_msgs__action__GetFeatures_SendGoal_Request__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__destroy(clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__are_equal(const clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * lhs, const clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_SendGoal_Request__Sequence__copy(
  const clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * input,
  clip_msgs__action__GetFeatures_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_SendGoal_Request * data =
      (clip_msgs__action__GetFeatures_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
clip_msgs__action__GetFeatures_SendGoal_Response__init(clip_msgs__action__GetFeatures_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    clip_msgs__action__GetFeatures_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_SendGoal_Response__fini(clip_msgs__action__GetFeatures_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
clip_msgs__action__GetFeatures_SendGoal_Response__are_equal(const clip_msgs__action__GetFeatures_SendGoal_Response * lhs, const clip_msgs__action__GetFeatures_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_SendGoal_Response__copy(
  const clip_msgs__action__GetFeatures_SendGoal_Response * input,
  clip_msgs__action__GetFeatures_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_SendGoal_Response *
clip_msgs__action__GetFeatures_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_SendGoal_Response * msg = (clip_msgs__action__GetFeatures_SendGoal_Response *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_SendGoal_Response));
  bool success = clip_msgs__action__GetFeatures_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_SendGoal_Response__destroy(clip_msgs__action__GetFeatures_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__init(clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_SendGoal_Response * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_SendGoal_Response *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__fini(clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_SendGoal_Response__Sequence *
clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * array = (clip_msgs__action__GetFeatures_SendGoal_Response__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__destroy(clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__are_equal(const clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * lhs, const clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_SendGoal_Response__Sequence__copy(
  const clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * input,
  clip_msgs__action__GetFeatures_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_SendGoal_Response * data =
      (clip_msgs__action__GetFeatures_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
clip_msgs__action__GetFeatures_GetResult_Request__init(clip_msgs__action__GetFeatures_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    clip_msgs__action__GetFeatures_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_GetResult_Request__fini(clip_msgs__action__GetFeatures_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
clip_msgs__action__GetFeatures_GetResult_Request__are_equal(const clip_msgs__action__GetFeatures_GetResult_Request * lhs, const clip_msgs__action__GetFeatures_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_GetResult_Request__copy(
  const clip_msgs__action__GetFeatures_GetResult_Request * input,
  clip_msgs__action__GetFeatures_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_GetResult_Request *
clip_msgs__action__GetFeatures_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_GetResult_Request * msg = (clip_msgs__action__GetFeatures_GetResult_Request *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_GetResult_Request));
  bool success = clip_msgs__action__GetFeatures_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_GetResult_Request__destroy(clip_msgs__action__GetFeatures_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_GetResult_Request__Sequence__init(clip_msgs__action__GetFeatures_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_GetResult_Request * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_GetResult_Request *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_GetResult_Request__Sequence__fini(clip_msgs__action__GetFeatures_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_GetResult_Request__Sequence *
clip_msgs__action__GetFeatures_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_GetResult_Request__Sequence * array = (clip_msgs__action__GetFeatures_GetResult_Request__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_GetResult_Request__Sequence__destroy(clip_msgs__action__GetFeatures_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_GetResult_Request__Sequence__are_equal(const clip_msgs__action__GetFeatures_GetResult_Request__Sequence * lhs, const clip_msgs__action__GetFeatures_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_GetResult_Request__Sequence__copy(
  const clip_msgs__action__GetFeatures_GetResult_Request__Sequence * input,
  clip_msgs__action__GetFeatures_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_GetResult_Request * data =
      (clip_msgs__action__GetFeatures_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "clip_msgs/action/detail/get_features__functions.h"

bool
clip_msgs__action__GetFeatures_GetResult_Response__init(clip_msgs__action__GetFeatures_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!clip_msgs__action__GetFeatures_Result__init(&msg->result)) {
    clip_msgs__action__GetFeatures_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_GetResult_Response__fini(clip_msgs__action__GetFeatures_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  clip_msgs__action__GetFeatures_Result__fini(&msg->result);
}

bool
clip_msgs__action__GetFeatures_GetResult_Response__are_equal(const clip_msgs__action__GetFeatures_GetResult_Response * lhs, const clip_msgs__action__GetFeatures_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!clip_msgs__action__GetFeatures_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_GetResult_Response__copy(
  const clip_msgs__action__GetFeatures_GetResult_Response * input,
  clip_msgs__action__GetFeatures_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!clip_msgs__action__GetFeatures_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_GetResult_Response *
clip_msgs__action__GetFeatures_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_GetResult_Response * msg = (clip_msgs__action__GetFeatures_GetResult_Response *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_GetResult_Response));
  bool success = clip_msgs__action__GetFeatures_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_GetResult_Response__destroy(clip_msgs__action__GetFeatures_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_GetResult_Response__Sequence__init(clip_msgs__action__GetFeatures_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_GetResult_Response * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_GetResult_Response *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_GetResult_Response__Sequence__fini(clip_msgs__action__GetFeatures_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_GetResult_Response__Sequence *
clip_msgs__action__GetFeatures_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_GetResult_Response__Sequence * array = (clip_msgs__action__GetFeatures_GetResult_Response__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_GetResult_Response__Sequence__destroy(clip_msgs__action__GetFeatures_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_GetResult_Response__Sequence__are_equal(const clip_msgs__action__GetFeatures_GetResult_Response__Sequence * lhs, const clip_msgs__action__GetFeatures_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_GetResult_Response__Sequence__copy(
  const clip_msgs__action__GetFeatures_GetResult_Response__Sequence * input,
  clip_msgs__action__GetFeatures_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_GetResult_Response * data =
      (clip_msgs__action__GetFeatures_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "clip_msgs/action/detail/get_features__functions.h"

bool
clip_msgs__action__GetFeatures_FeedbackMessage__init(clip_msgs__action__GetFeatures_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    clip_msgs__action__GetFeatures_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!clip_msgs__action__GetFeatures_Feedback__init(&msg->feedback)) {
    clip_msgs__action__GetFeatures_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
clip_msgs__action__GetFeatures_FeedbackMessage__fini(clip_msgs__action__GetFeatures_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  clip_msgs__action__GetFeatures_Feedback__fini(&msg->feedback);
}

bool
clip_msgs__action__GetFeatures_FeedbackMessage__are_equal(const clip_msgs__action__GetFeatures_FeedbackMessage * lhs, const clip_msgs__action__GetFeatures_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!clip_msgs__action__GetFeatures_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_FeedbackMessage__copy(
  const clip_msgs__action__GetFeatures_FeedbackMessage * input,
  clip_msgs__action__GetFeatures_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!clip_msgs__action__GetFeatures_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

clip_msgs__action__GetFeatures_FeedbackMessage *
clip_msgs__action__GetFeatures_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_FeedbackMessage * msg = (clip_msgs__action__GetFeatures_FeedbackMessage *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(clip_msgs__action__GetFeatures_FeedbackMessage));
  bool success = clip_msgs__action__GetFeatures_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
clip_msgs__action__GetFeatures_FeedbackMessage__destroy(clip_msgs__action__GetFeatures_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    clip_msgs__action__GetFeatures_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__init(clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_FeedbackMessage * data = NULL;

  if (size) {
    data = (clip_msgs__action__GetFeatures_FeedbackMessage *)allocator.zero_allocate(size, sizeof(clip_msgs__action__GetFeatures_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = clip_msgs__action__GetFeatures_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        clip_msgs__action__GetFeatures_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__fini(clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      clip_msgs__action__GetFeatures_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

clip_msgs__action__GetFeatures_FeedbackMessage__Sequence *
clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * array = (clip_msgs__action__GetFeatures_FeedbackMessage__Sequence *)allocator.allocate(sizeof(clip_msgs__action__GetFeatures_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__destroy(clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__are_equal(const clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * lhs, const clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!clip_msgs__action__GetFeatures_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
clip_msgs__action__GetFeatures_FeedbackMessage__Sequence__copy(
  const clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * input,
  clip_msgs__action__GetFeatures_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(clip_msgs__action__GetFeatures_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    clip_msgs__action__GetFeatures_FeedbackMessage * data =
      (clip_msgs__action__GetFeatures_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!clip_msgs__action__GetFeatures_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          clip_msgs__action__GetFeatures_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!clip_msgs__action__GetFeatures_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
