...
constexpr auto kMaxEntryId = MaxValue<EntryId>::value;
...
public:
  ...
  void query(const std::vector<cv::Mat>& features, QueryResults& ret,
             const std::unordered_set<EntryId>& rm_ids, int max_results = 1, EntryId max_id = kMaxEntryId) const;
  /**
   * Queries the database with some features
   ...
   *   < 0 means all
   */
  void query(const cv::Mat& features, QueryResults& ret, const std::unordered_set<EntryId>& rm_ids,
             int max_results = 1, EntryId max_id = kMaxEntryId) const;
   ...
   *   < 0 means all
   */
  void query(const BowVector& vec, QueryResults& ret, const std::unordered_set<EntryId>& rm_ids,
             int max_results = 1, EntryId max_id = kMaxEntryId) const;
...
protected:
  ...
  void queryL1(const BowVector& vec, QueryResults& ret, const std::unordered_set<EntryId>& rm_ids,
               int max_results, EntryId max_id) const;

  /// Query with L2 scoring
  void queryL2(const BowVector& vec, QueryResults& ret, const std::unordered_set<EntryId>& rm_ids,
               int max_results, EntryId max_id) const;

  /// Query with Chi square scoring
  void queryChiSquare(const BowVector& vec, QueryResults& ret,
                      const std::unordered_set<EntryId>& rm_ids, int max_results, EntryId max_id) const;
  ...
  void queryKL(const BowVector& vec, QueryResults& ret, const std::unordered_set<EntryId>& rm_ids,
               int max_results, EntryId max_id) const;
